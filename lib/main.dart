...
class _ExoplanetPageState extends State<ExoplanetPage> {
  ...
  Future<void> fetchExoplanets() async {
    ...
    try {
      // URL langsung dan URL proxy untuk menangani CORS
      final List<String> proxyUrls = [
        // URL Direct tanpa proxy - paling cocok untuk mobile
        ...
        'https://corsproxy.io/?${Uri.encodeComponent('https://exoplanetarchive.ipac.caltech.edu/TAP/sync?query=select+top+10+pl_name,hostname,disc_year,pl_orbper,pl_bmassj,pl_radj+from+ps&format=json')}'
      ];

      // Tambahkan header khusus untuk mengatasi masalah di mobile
      final headers = {
      ...
      for (final url in proxyUrls) {
        try {
          ...
          if (response.statusCode == 200) {
            try {
              // Cek apakah response dari proxy atau langsung
              final dynamic responseData;
              if (url.contains('allorigins') || url.contains('corsproxy')) {
                final body = jsonDecode(response.body);
                responseData = body['contents'] != null
              ...
              if (responseData['metadata'] != null && responseData['data'] != null) {
                final columns = responseData['metadata'];
                ...
                for (var row in rows) {
                  ...
                  for (int i = 0; i < columns.length; i++) {
                    var value = row[i];
                    // Konversi nilai string ke tipe yang sesuai
                    if (columns[i]['name'] == 'disc_year' && value is String) {
                      value = int.tryParse(value) ?? 0;
                    }
                    if (['pl_orbper', 'pl_bmassj', 'pl_radj'].contains(columns[i]['name']) &&
                        value is String) {
                ...
                debugPrint('Berhasil memuat data dari API');
                setState(() {
                  exoplanets = parsed;
                  isLoading = false;
                });
                ...
                lastError = 'Format data tidak valid';
                debugPrint('Format data tidak valid dari $url');
              }
            } catch (parseErr) {
              lastError = 'Error parsing: ${parseErr.toString()}';
              debugPrint('Parse error dari $url: $parseErr');
              continue;
            }
          } else {
            lastError = 'Status code: ${response.statusCode}';
            debugPrint('Bad status dari $url: ${response.statusCode}');
          }
        } catch (e) {
          lastError = e.toString();
          debugPrint('Request error dari $url: $e');
          continue;
        }
      }

      if (!success) {
        debugPrint('Semua request gagal, menggunakan data lokal. Error terakhir: $lastError');
        await loadLocalData();
      }
    } catch (e) {
  ...
  Future<void> loadLocalData() async {
    debugPrint('Loading local data...');
    try {
      // Tambahkan penanganan error yang lebih baik
      String jsonString;
      try {
        jsonString = await DefaultAssetBundle.of(context).loadString('assets/exoplanets.json');
      } catch (assetError) {
        debugPrint('Error loading asset: $assetError');
        // Fallback data jika file tidak ditemukan
        jsonString = '''[
        {"pl_name":"Fallback Planet 1","hostname":"Emergency Star","disc_year":2000,"pl_orbper":3.5,"pl_bmassj":1.2,"pl_radj":1.1},
        {"pl_name":"Fallback Planet 2","hostname":"Emergency Star","disc_year":2001,"pl_orbper":5.2,"pl_bmassj":0.8,"pl_radj":0.9}
      ]''';
      }

      final List<dynamic> data = jsonDecode(jsonString);
      debugPrint('Loaded ${data.length} planets from local data');
      ...
      setState(() {
        ...
        isLoading = false;
        // Tampilkan UI error yang lebih baik
        exoplanets = [];
      });
    }
  }
